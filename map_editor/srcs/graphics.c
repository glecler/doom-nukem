...
int	ft_set_rgb_type(unsigned char *rgb, int type, int dmode)
{
	...
	else if ((type == FLOOR && (dmode == ALL || dmode == FLOOR))
		|| (type == TOP_FLOOR_WALL && dmode == FLOOR) || (type ==
			TOP_FLOOR && dmode == FLOOR) || (type == FLOOR_WALL &&
			dmode == FLOOR))
		ft_set_rgb(rgb, 120, 240, 120);
	else if ((type == WALL && (dmode == ALL || dmode == WALL))
		|| (type == TOP_FLOOR_WALL && dmode == WALL) || (type ==
			FLOOR_WALL && dmode == WALL) || (type == TOP_WALL &&
			dmode == WALL))
...
int	ft_update_map(t_e_data *e_data)
{
	...
	while (link_buff)
	{
		...
		ft_set_rgb_type(e_data->win_data->rgb, link_buff->type,
			e_data->display_mode);
